...
λ = st.sidebar.number_input('λ', min_value=0.0 , max_value=4.0 , value=4.00 , format="%.16f")
N = st.sidebar.number_input('N', min_value=50 , max_value=10000 , value=9999) 
x = np.zeros(N)
x[0] = st.sidebar.number_input('x0', min_value=0.001, max_value=0.999, value=0.890 , format="%.16f")
...
if loop:
    ...
    for p in pair_x : 
        ...
        pair.append(p)
        cf.append(delta_df['cf_usd'][-1])
    df_dict = {'data': data, 'p': pair, 'cf': cf} 
